...
| Hardware Simulator | C++ HDL parser, hierarchical net graph, 35 embedded declarations, built-ins, clocks, pins/components/memory and ROM controls | 29 bundled + 7 independent reference cases; confirmed narrow-negative mismatch and remaining scheduling/GUI gaps in hardware.md |
...
checks, incomplete script string variables and the confirmed narrow-negative HDL mismatch. Partial output persistence on script errors now has a rejection regression.
