        ...
        for(std::size_t p=begin;p<end&&result.passed;){
            ...
            try{
                if(op=="repeat"||op=="while"){
                    ...
                    for(auto& f:formats){std::string value;if(f.type=='S'&&useHardware)value=hardware.getText(f.name);else{int n=get(f.name);value=std::to_string(n);if(f.type=='B')value=std::bitset<16>(Word(n)).to_string();else if(f.type=='X'){std::ostringstream s;s<<std::hex<<std::uppercase<<std::setw(4)<<std::setfill('0')<<Word(n);value=s.str();}}if(value.size()>std::size_t(f.width))value=value.substr(value.size()-f.width);int extra=f.width-int(value.size());row+=std::string(f.left+(f.type=='S'?0:extra),' ')+value+std::string(f.right+(f.type=='S'?extra:0),' ')+"|";}output(row);
