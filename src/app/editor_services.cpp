...
QVariantMap EditorServices::replaceOne(QQuickTextDocument* q,const QString& query,const QString& replacement,int from,bool sensitive,bool word){auto* d=doc(q);if(!d)return {{"start",-1},{"end",-1}};auto m=find(d->toPlainText(),query,from,sensitive,word);int p=m["start"].toInt();if(p<0)return m;QTextCursor c(d);c.beginEditBlock();c.setPosition(p);c.setPosition(m["end"].toInt(),QTextCursor::KeepAnchor);c.insertText(replacement);c.endEditBlock();return {{"start",p},{"end",p+replacement.size()}};}
